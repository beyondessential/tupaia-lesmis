...
export class IndicatorCacheCleaner {
  ...
  handleEntityRelationChange = async ({ old_record: oldRecord, new_record: newRecord }) => {
    ...
    const tasks = [oldRecord, newRecord]
      .filter(r => r)
      .map(r => this.scheduleIndicatorCacheClean(r.entity_hierarchy_id, r.parent_id));
  ...
  handleEntityHierarchyChange = async ({
    ...
    const tasks = projectsUsingHierarchy.map(p =>
      this.scheduleIndicatorCacheClean(hierarchyId, p.entity_id),
    );
    return Promise.all(tasks);
  };
  ...
  scheduleIndicatorCacheClean(analytic) {
