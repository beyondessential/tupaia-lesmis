...
const getPermissionListWithWildcard = (accessPolicy, countryCodes) => {
...
export class DataBroker {
  ...
  getUserPermissions(countryCodes) {
  ...
  getCountryCodes = async organisationUnitCodes => {
  ...
  checkDataElementPermissions = async (dataElements, countryCodes) => {
    const allUserPermissions = this.getUserPermissions();
    if (allUserPermissions.includes(BES_ADMIN_PERMISSION_GROUP)) {
      return true;
    }

    const getDataElementsWithMissingPermissions = permissions =>
      dataElements
    ...
    const missingPermissionsPerCountry = {};
  ...
  checkDataGroupPermissions = async (dataGroups, countryCodes) => {
