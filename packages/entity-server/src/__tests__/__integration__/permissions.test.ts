...
describe('permissions', () => {
  ...
  describe('filtering entities by acesss', () => {
    it('filters entities when requestion for some with access', async () => {
      const { text } = await app.post('hierarchy/goldsilver', {
        query: { fields: 'code,name,type' },
        body: { entities: ['redblue', 'KANTO', 'VIRIDIAN', 'ECRUTEAK', 'SLOWPOKE_WELL'] },
      });
    ...
    it('filters descendants when requestion for some with access', async () => {
    ...
    it('filters relatives when requestion for some with access', async () => {
    ...
    it('filters relationships when requestion for some with access', async () => {
