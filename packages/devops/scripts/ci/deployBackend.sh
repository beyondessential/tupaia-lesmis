...
if curl --output /dev/null --silent --head --fail $DEPLOYMENT_URL; then
    ...
    ssh -o ServerAliveInterval=15 ubuntu@$DEPLOYMENT_URL "cd tupaia/packages/${PACKAGE} && yarn build && pm2 delete ${PACKAGE} || true && pm2 start --name ${PACKAGE} 'yarn start-post-build' --wait-ready --listen-timeout 15000 --time;"
else
