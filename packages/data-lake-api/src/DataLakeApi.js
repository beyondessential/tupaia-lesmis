...
export class DataLakeApi {
  async fetchEvents(optionsInput) {
    ...
    const results = await new EventsFetchQuery(getDatabase(), options).fetch();
  ...
  async fetchDataElementsMetadataFromSqlQuery(sqlQuery, includeOptions) {
    const dataElementsMetadata = await sqlQuery.executeOnDatabase(getDatabase());
