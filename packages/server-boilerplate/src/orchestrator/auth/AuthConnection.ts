...
const basicAuthHandler = {
  getAuthHeader: async () => {
    const { CENTRAL_API_CLIENT_NAME, CENTRAL_API_CLIENT_PASSWORD } = process.env;
    const CENTRAL_API_CREDENTIALS = `${CENTRAL_API_CLIENT_NAME}:${CENTRAL_API_CLIENT_PASSWORD}`;
    return `Basic ${Buffer.from(CENTRAL_API_CREDENTIALS).toString('base64')}`;
  },
};
